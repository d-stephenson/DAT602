...
from `city`
where `District` = 'Auckland'
order by `Name`; 

select sum(`Population`) as 'Auc Pop'
from `city`
where `District` = 'Auckland'
order by `Name`; 

select `District`, sum(`Population`) as `Dis Pop`
from `city`
group by `District`
having `Dis Pop` >= 10000000 
order by `District`; 

-- Difference in SQL Server (SQL Server should be [] instead of back slashes, or nothing at all as below)

select District, sum(Population) as DisPop
from city
group by District
